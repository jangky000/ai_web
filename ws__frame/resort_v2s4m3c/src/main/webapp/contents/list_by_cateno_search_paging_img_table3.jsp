...
<jsp:include page="/menu/top.jsp" flush='false' />
  ...
  <form name='frm' id='frm' method='get' action='./list.do'>
    ...
    <ASIDE style='float: right;'>
      ...
      <c:choose>
        <c:when test="${param.word != '' }"> <!-- 검색을 하는 경우 -->
          <input type='text' name='word' id='word' value='${param.word }' 
                     style='width: 35%;'>
        </c:when>
        <c:otherwise> <!-- 검색 하지 않는 경우 -->
          <input type='text' name='word' id='word' value='' style='width: 35%;'>
        </c:otherwise>
      </c:choose>
      <button type='submit'>검색</button>
      <c:if test="${param.word.length() > 0 }"> <!-- 검색취소: redirect -->
        <button type='button' onclick="location.href='./list.do?cateno=${cateVO.cateno}&word='">검색 취소</button>  
      </c:if>
          ...
          <tr style="height: 90px;"> 
            ...
            <td style='vertical-align: middle;'>
              <c:choose>
                ...
                <c:when test="${ansnum > 0 }"> <%-- 자식글인 경우 아이콘 출력, 들여쓰기 --%>
                  <img src='./images/white.png' style='width: ${indent * 20}px;  height:5px; opacity: 0.0;'>
