...
<script type="text/javascript">
  ...
  $(function() { // 자동 실행
    ...
    list_by_contentsno(${contentsVO.contentsno });  // JS의 EL 접근 -> 댓글 목록 load
  ...
  function youtube_delete_modal(){
    $('#modal_panel_youtube').modal();    // youtube 삭제 요청 모달 출력
  }
  ...
  function youtube_delete_send() {
    ...
    var frm = $('#frm_youtube'); // Youtube 삭제 폼 // form이 여러개인 경우 id 중복 주의
  ...
  function panel_img(file) {
    ...
    $('#attachfile_panel').html(tag); // 문자열 태그 적용
      ...
      <fieldset class="fieldset">
        <ul>
          ...
          <c:if test="${contentsVO.youtube.trim().length() > 0 }">
            <li class='li_none' style='clear: both;'>
              <DIV style='width:900px; height: auto; margin: 0px auto; text-align: center;'>
                ${contentsVO.youtube }
              </DIV>
            </li>
          </c:if>
          ...
          <c:if test="${contentsVO.mp4.trim().length() > 0 }">
            <li class='li_none' style='clear: both;'>
              <DIV style='width:900px; height: auto; margin: 0px auto; text-align: center;'>
                <VIDEO controls src='./storage/mp4/${contentsVO.mp4 }' style='width: 70%; background-color: #EEEEEE;'></VIDEO>
              </DIV>
            </li>
          </c:if>
