...
public class CategrpCont {
  ...
  public ModelAndView create(CategrpVO categrpVO) {
    ...
    if(cnt == 1) {
      ...
      // redirect기법으로 처리(request 객체 사라짐)
    } else {
      mav.setViewName("/categrp/create_msg.jsp"); // webapp/categrp/create_msg.jsp
    }
 ...
 public ModelAndView update(CategrpVO categrpVO) { // 파라미터가 들어옴
  ...
  //http://localhost:9090/ojt/categrp/read_delete.do?categrpno=1
  /**
   * 한 건 조회, 수정 폼
   * @param categrpno 조회할 카테고리 번호
   * @return mav
  ...
  public ModelAndView delete(int categrpno) { // 파라미터가 들어옴
    ...
    if(cnt == 1) {
      // List<CategrpVO> list = this.categrpProc.list_categrpno_asc(); //삭제 후 목록
      // mav.addObject("list", list); // request 객체에 저장
      mav.setViewName("redirect:/categrp/list.do"); // 스프링 재호출
      // redirect기법으로 처리(request 객체 사라짐)
    } else {
      mav.addObject("categrpVO", categrpVO); // --> 무엇을 삭제하지 못했는지 보여줌
      mav.setViewName("/categrp/delete_msg.jsp"); // webapp/categrp/delete_msg.jsp
    }
