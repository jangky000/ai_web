...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    // 현재 2그룹일 경우: (2 - 1) * 10 = 1 그룹의 마지막 페이지: 10
    // 현재 3그룹일 경우: (3 - 1) * 10 = 2 그룹의 마지막 페이지 20
    ...
    for(int i=startPage; i<=endPage; i++){ 
      ...
      if (nowPage == i){ // 페이지가 현재 페이지와 같다면 CSS 강조
        str.append("<span class='span_box_2'>"+i+"</span>"); // 현재 페이지, 강조 
      }else{
        // 현재 페이지가 아닌 페이지는 이동이 가능하도록 anchor 링크 설정
