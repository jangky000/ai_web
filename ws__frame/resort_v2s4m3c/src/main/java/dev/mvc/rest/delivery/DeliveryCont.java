...
public class DeliveryCont {
  ...
  public ResponseEntity<Integer> create(@RequestBody DeliveryVO vo) { // requestbody: jquery에서 받은 json을 바로 받을 수 있음
    System.out.println("등록 요청됨: " + vo.toString());
    int cnt =1;
    
 // 객체화 해서 출력: int -> new Integer(cnt) 이렇게 하지 않으면 에러 발생
    ResponseEntity<Integer> entity = new ResponseEntity<Integer>(new Integer(cnt), HttpStatus.OK);
    return entity;
  }
  
  // 다른 서버로 접근 가능
  // http://172.16.12.3:9090/resort/delivery_rest/list/A001/1/2020-001
  // http://172.16.12.4:9090/resort/delivery_rest/list/A001/1/2020-001
   ...
   * @return
   */
  @RequestMapping(value="/delivery_rest/list/{key}/{page}/{code}", method=RequestMethod.GET)
   ...
   * @return
   */
  @RequestMapping(value="/delivery_rest/read/{deliveryno}", method=RequestMethod.GET)
   ...
   * @return
   */
  @RequestMapping(value = "/delivery_rest/delete/{deliveryno}", method = RequestMethod.DELETE)
