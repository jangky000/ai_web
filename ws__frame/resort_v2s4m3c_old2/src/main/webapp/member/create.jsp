...
<script type="text/javascript">
  ...
  function checkID() {
    var frm = $('#frm');
    var id = $('#id', frm).val(); // frm 폼에서 id를 찾아서 값을 가져옴
    var params = '';
    var msg = '';
    
    if ($.trim(id).length == 0) { // id를 입력 받지 않은 경우
      // 자바스크립트에서는 줄바꿈 \n이지만 html에 최종 출력되는 메시지에는 <br>을 사용해야함
      msg = "ID를 입력하세요.<br>ID 입력은 필수입니다.<br>ID는 3자리 이상 권장.";
